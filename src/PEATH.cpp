...
typedef struct {			// Haplotype sequeucne
	char *seq, *seq2;
	double sum_D;			// Weighted MEC (distance)
}HaploType;
...
//
// calculating w-MEC values (position version) used only in xxx_switch_procedure( )
...
//
// calculate w-MEC values (2seq version)
...
// a haplotype sequence is a chromosome in GA 
// -w-MEC value is the fitness valued used in GA.
//
void GA(HaploType &besthaplo, BlockType &block) {
	...
	for (int i = 0; i<POPSIZE; i++) {	// initialize population randomly
		...
		calc_sumD(Population[i], block, 0);	// compute population[i].sum_D. The fitness value of each individual is -sum_D.
	} // end_for

	sort(Population, Population+POPSIZE, compare_sumD_val);	// sorting with w-MEC values
	...
	while (g_iter--) {
		...
		for (int i = OFFSIZE; i<POPSIZE - 1; i++)		// calculate the w-MEC values of new individuals
...
int range_switch_toggling(HaploType & haplo, BlockType &block) {
	...
	while (isImp) {		// while finding better solution
		...
		for (int i = 0; i<block.length; i++) {	// for each position in block
			...
			// calculating the w-MEC value assuming that haplo.seq[i] is toggled
	...
	while (isImp) {		// while finding better solution
		...
		for (int is = 0; is<block.length; is++) {	// for each position in block
			...
			for (int ie = is; ie < block.length; ie++) {	// for each position in block
				...
				// calculating the w-MEC value assuming that haplo.seq[ie] is toggled
